...
We evaluate two models at each step: The first including auction variables and oil and FTSE prices. The second including only auction variables. 
...
We can see that the average number of bids per bidder is now significant in the absence of oil prices and FTSE, even if its effect is small. Auction volume still seems to have very little effect on price. The models have very comparable AIC's (differing by a paltry 0.2) so its difficult to conclude which model may better serve us. Perhaps our predictions later shall help us sort that out. 
...
Before we get stuck into the weeds with these, it's important to have a quick discussion about lags:
For the folowing data, lag has to be recalculated and adjusted as we cannot just subset the lags from the entire dataset and have them line up correctly.  
...
EU_no_pool_fit_auction_only<- glm(Auction_Price_fd ~ Max_bid_fd +Avg_bids_fd+
                          Auction_vol_fd, 
                 family=gaussian, data=EU_only)
...
DE_no_pool_fit_auction_only<- glm(Auction_Price_fd ~ Max_bid_fd +Avg_bids_fd+
                          Auction_vol_fd, 
                 family=gaussian, data=DE_only)
...
PL_no_pool_fit_auction_only<- glm(Auction_Price_fd ~ Max_bid_fd +Avg_bids_fd+
                          Auction_vol_fd, 
                 family=gaussian, data=PL_only)
...
EUAA_no_pool_fit_auction_only<- glm(Auction_Price_fd ~ Max_bid_fd +Avg_bids_fd+
                          Auction_vol_fd, 
                 family=gaussian, data=EUAA_only)
...
```
These mdoels too, suffer from the same issues as for poland and Germany. 

Perhaps our two Partial Pooling models (with oil and ftse and without them) can do better.
