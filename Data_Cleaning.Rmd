...
#Auction Price Lag
Auction_Price_lag<- EU_only_auction$`Auction Price €/tCO2`[2:length(EU_only_auction$`Auction Price €/tCO2`)-1]
Auction_Price_lag<- c(0,Auction_Price_lag)
Auction_Price_lag[length(Auction_Price_lag)]<-0
EU_only_with_lag<- EU_only_auction |> mutate(
                                          Auction_Price_lag= Auction_Price_lag, .after=`Auction Price €/tCO2`
)

#Maximum bid lag
Max_bid_lag<- EU_only_auction$`Maximum Bid €/tCO2`[2:length(EU_only_auction$`Maximum Bid €/tCO2`)-1]
Max_bid_lag<- c(0,Max_bid_lag)
Max_bid_lag[length(Max_bid_lag)]<-0
EU_only_with_lag<- EU_only_with_lag |> mutate(
                                          Max_bid_lag= Max_bid_lag, .after=`Maximum Bid €/tCO2`
)

#Avg bids per bidder lag
Avg_num_bids_per_bidder_lag<- EU_only_auction$`Average number of bids per bidder`[2:length(EU_only_auction$`Average number of bids per bidder`)-1]
Avg_num_bids_per_bidder_lag<- c(0,Avg_num_bids_per_bidder_lag)
Avg_num_bids_per_bidder_lag[length(Avg_num_bids_per_bidder_lag)]<-0
EU_only_with_lag<- EU_only_with_lag |> mutate(
                                          Avg_num_bids_per_bidder_lag= Avg_num_bids_per_bidder_lag, .after=`Average number of bids per bidder`
)

#Avg vol per bidder
Avg_vol_per_bidder_lag<- EU_only_auction$`Average volume bid per bidder`[2:length(EU_only_auction$`Average volume bid per bidder`)-1]
Avg_num_bids_per_bidder_lag<- c(0,Avg_num_bids_per_bidder_lag)
Avg_num_bids_per_bidder_lag[length(Avg_num_bids_per_bidder_lag)]<-0
EU_only_with_lag<- EU_only_with_lag |> mutate(
...
oil_price_lag<- EU_only_auction$oil_price[2:length(EU_only_auction$oil_price)-1]
oil_price_lag<- c(0,oil_price_lag)
oil_price_lag[length(oil_price_lag)]<-0
EU_only_with_lag<- EU_only_with_lag |> mutate(
...
ftse_price_lag<- EU_only_auction$ftse_price[2:length(EU_only_auction$ftse_price)-1]
ftse_price_lag<- c(0,ftse_price_lag)
ftse_price_lag[length(ftse_price_lag)]<-0
EU_only_with_lag<- EU_only_with_lag |> mutate(
...
Auction_vol_lag<- EU_only_auction$`Auction Volume tCO2`[2:length(EU_only_auction$`Auction Volume tCO2`)-1]
Auction_vol_lag<- c(0,Auction_vol_lag)
Auction_vol_lag[length(Auction_vol_lag)]<-0
EU_only_with_lag<- EU_only_with_lag |> mutate(
...
#PL lag:
PL_lag<- PL_only_with_lag |> select('Date', 'AuctionType', 'lag')
ets_EU_DE_PL_with_lag<- merge(x=ets_EU_DE_with_lag, 
                              y= PL_lag, by = c('Date', 'AuctionType'),
                              all.x=TRUE)

ets_EU_DE_PL_with_lag$lag.x<- ifelse(is.na(ets_EU_DE_PL_with_lag$lag.x), 
                                  ets_EU_DE_PL_with_lag$lag.y,
                                  ets_EU_DE_PL_with_lag$lag.x)

ets_EU_DE_PL_with_lag<- ets_EU_DE_PL_with_lag |> rename('lag'='lag.x') |> select(-'lag.y')

#Adding EUAA lag:

EUAA_lag<- EUAA_only_with_lag |> select('Date', 'AuctionType', 'lag')
